...
public const float TRACK_SPACING_X = 100f;
    ...
    private void Awake()
    {
        ...
        try
        {
            ProfileFlagsService.Instance.StartServer();
            YargLogger.LogInfo("ProfileFlagsService server started.");
        }
        catch (Exception ex)
        {
            YargLogger.LogException(ex, "Failed to start ProfileFlagsService on GameManager Awake.");
        }

        // Clear any previously known profiles from the flag service
        ProfileFlagsService.Instance.ClearRegisteredProfiles();
        // Register current players (make sure YargPlayers is populated before this)
        if (YargPlayers != null && YargPlayers.Count > 0) {
            ProfileFlagsService.Instance.RegisterProfiles(YargPlayers.Select(p => p.Profile.Id));
            YargLogger.LogInfo($"Registered {YargPlayers.Count} players with ProfileFlagsService.");
        } else {
             YargLogger.LogWarning("YargPlayers list is null or empty in Awake, cannot register profiles with ProfileFlagsService yet.");
        }
        // --- ADDITIONS END ---
    }
